...
class Booking extends CI_Controller
{
    ...
    public function index()
    {
        ...
        foreach ($user as $a) {
            $data = [
                ...
                'user' => $user['name'],
                'email' => $user['email'],
                'tanggal_input' => $user['date_created']
            ];
        }

        // Variabel $dtb yang isinya diambil dari jumlah data booking berdasarkan id
    ...
    public function tambahBooking()
    {
        ...
        if ($databooking > 0) {
            $this->session->set_flashdata('message', '<div class="alert alert-danger alertmessage" role="alert">Masih Ada booking buku sebelumnya yang belum diambil.<br> Ambil Buku yang dibooking atau tunggu 1x24 Jam untuk bisa booking kembali </div>');
            redirect(base_url());
        }
